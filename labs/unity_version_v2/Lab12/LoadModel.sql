...
CREATE EXTERNAL DATA SOURCE [ext_datasource_with_abfss] WITH (
	TYPE = HADOOP,
	LOCATION = 'abfss://synapse@<INSERT STORAGE ACCOUNT NAME>.dfs.core.windows.net',
...
CREATE EXTERNAL FILE FORMAT parquet_format WITH (
	...
	LOCATION = 'fact_sales.snappy.parquet',
...
CREATE EXTERNAL TABLE dim_date (
	...
	LOCATION = 'dim_date.snappy.parquet',
	DATA_SOURCE = [ext_datasource_with_abfss],
	FILE_FORMAT = [parquet_format]
)
GO
	CREATE EXTERNAL TABLE dim_products (
		...
		LOCATION = 'dim_products.snappy.parquet',
		DATA_SOURCE = [ext_datasource_with_abfss],
		FILE_FORMAT = [parquet_format]
	)
GO
	CREATE EXTERNAL TABLE dim_customers (
		...
		LOCATION = 'dim_customers.snappy.parquet',
		DATA_SOURCE = [ext_datasource_with_abfss],
		FILE_FORMAT = [parquet_format]
	)
GO
	CREATE EXTERNAL TABLE dim_catseg (
		...
		LOCATION = 'dim_catseg.snappy.parquet',
		DATA_SOURCE = [ext_datasource_with_abfss],
		FILE_FORMAT = [parquet_format]
	)
GO
